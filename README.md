...
## Возможности
- **Интерактивные меню Matrix и выполнение скриптов (v3.1.2)**: Текстовые интерактивные меню команд (например, `!snapshot/hd` или `!stats`) с поддержкой быстрого запуска по эмодзи-реакциям (`m.reaction`). Бот поддерживает как выполнение внешних HTTP-запросов, так и запуск локальных скриптов (из папки скриптов внутри контейнера) с настраиваемым таймаутом `SCRIPT_TIMEOUT` (скрипты имеют приоритет над URL).
- **Красивое форматирование и обработка ответов команд (v3.1.2)**: Бот автоматически распознает и красиво форматирует JSON и XML ответы от команд. При форматировании JSON-объектов соблюдается исходный порядок полей (через `DecodeOrderedJSON`), а поле `text` (при наличии на верхнем уровне) всегда выводится первым без префикса ключа.
- **Безопасное логирование и маскирование (v3.1.2)**: При сбоях выполнения команд чат Matrix защищен от утечек — выводится строго одно лаконичное сообщение `❌ Ошибка при выполнении команды !<имя_команды>`. Все подробные технические детали (ошибки сетевых запросов, статус-коды, `stderr` скриптов) записываются в системный лог бота с автоматическим маскированием паролей и URL-параметров.
...
- `menu` - уникальный идентификатор подключенного меню интерактивных команд (только для Matrix). Наличие меню активирует фоновое прослушивание сообщений и реакций.
...
services:
  notify-bot:
    ...
    environment:
      ...
      - SCRIPT_TIMEOUT=${SCRIPT_TIMEOUT:-15}
      - TZ=${TZ:-UTC}
      # - TZ=Europe/Moscow
