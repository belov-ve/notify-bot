...
**Версия 3.1.1 (Форматирование ответов команд)**

## Возможности
- **Обработка и форматирование HTTP-ответов команд меню (v3.1.1)**: Если при выполнении команды из меню внешняя система возвращает контент, бот выводит его в Matrix-чат. Если возвращен простой текст, он выводится без изменений. Если возвращена сложная JSON или XML структура, бот производит её рекурсивный обход и красивое форматирование с отступами и списками (у XML автоматически отсекается корневой тег-контейнер). Лимит считываемого тела ответа ограничен 1 МБ.
...
- **Интерактивные меню Matrix (v3.1.1)**: Текстовые интерактивные меню команд (например, `!snapshot/hd` или `!snapshot/sd`) с выполнением HTTP-запросов на стороне сервера бота и поддержкой быстрого запуска по эмодзи-реакциям (`m.reaction`).
