...
**Версия 3.1.0 (Интерактивные меню)**
...
- **Интерактивные меню Matrix (v3.1.0)**: Текстовые интерактивные меню команд (например, `!snapshot/hd` или `!snapshot/sd`) с выполнением HTTP-запросов на стороне сервера бота и поддержкой быстрого запуска по эмодзи-реакциям (`m.reaction`).
  ...
  - id: "system_menu"
    items:
      - name: "status"
        ...
        reaction: "📊"
      - name: "uptime"
        url: "http://localhost:8090/uptime"
