...
│       ├── scheduler.go     # Планировщик периодических задач по расписанию (cron)
│       ├── telegram.go      # Интеграция с Telegram Bot API
│       ├── matrix.go        # Интеграция с Matrix (поддержка E2EE и Key Backup)
│       ├── utils.go         # Вспомогательные функции (Ordered JSON, IP)
│       ├── go.mod           # Зависимости проекта
│       └── go.sum           # Контрольные суммы зависимостей
├── data/
│   └── scripts/
│       ├── stats.sh         # Скрипт состояния очередей для планировщика
│       ├── weather.sh       # Скрипт прогноза погоды через wttr.in с HTML-выводом
│       ├── example_report.sh # Пример скрипта для планировщика с текстовым выводом
│       └── example_pdf_report.sh # Пример скрипта для планировщика с генерацией PDF
└── README.md
```
...
- **Отложенная доставка**: Сообщения, доставленные не с первой попытки (при `attempts > 0` или статусе `"failed"`), ВСЕГДА помечаются префиксом `[Отложенная доставка]` с указанием оригинального времени создания и часовым поясом. Добавление префикса происходит независимо от настройки `show_time`, а дублирование меток времени полностью исключено.
...
- **Метки времени (ShowTime)**: Добавление метки времени приема сообщения ботом (YYYY-MM-DD HH:MM:SS MST) к тексту уведомления при обычной (первичной) успешной доставке без задержек.
- **Защита воркеров от блокировок**: При возникновении сетевой ошибки воркер мгновенно прерывает выгонку очереди текущего канала (`break`), ставя сообщение на паузу до времени ретрая `next_attempt_at` и не блокируя горутину таймаутами остальных сообщений.
...
| Поле      | Тип    | Описание                                                                 |
|-----------|--------|--------------------------------------------------------------------------|
| `text`    | string | Основное текстовое описание. Выводится первой строкой. Поддерживает HTML-разметку (см. подробнее ниже). |
...
### 1.1. Поддержка HTML-форматирования (parse_mode: HTML)
Бот поддерживает отправку уведомлений со сложным HTML-форматированием (жирный шрифт, курсив, ссылки, моноширинный текст).

Для включения режима парсинга HTML, текст в поле `text` (или подпись в multipart запросах) должен начинаться с префикса `<html>` и заканчиваться тегом `</html>`.
* **Telegram:** При наличии префикса вызовы API `/sendMessage` и `/sendPhoto` выполняются с параметром `"parse_mode": "HTML"`. HTML-теги автоматически сохраняются при разделении длинных подписей (>1024 символов) и текстов (>4096 символов).
* **Matrix:** Сообщение отправляется с типом форматирования `org.matrix.custom.html` и HTML-кодом в поле `formatted_body`. Для обычного plain-text отображения в поле `body` все теги автоматически удаляются.

**Поддерживаемые HTML-теги:**
* `<b>жирный текст</b>` или `<strong>жирный</strong>`
* `<i>курсив</i>` или `<em>курсив</em>`
* `<code>моноширинный текст</code>`
* `<pre>блок кода</pre>`
* `<a href="https://example.com">текст ссылки</a>`

*Пример запроса с HTML-разметкой:*
```json
{
  "text": "<html>📅 <b>Заголовок отчета</b>\nНа улице: <i>идет дождь</i>\n🌬 Скорость ветра: <code>5 м/с</code>\nПодробнее: <a href=\"https://wttr.in/Moscow\">wttr.in</a></html>"
}
```
...
instances:
  ...
  - name: "matrix_bot"
    ...
    matrix:
      ...
      recovery_key: "ваша_фраза_восстановления" # Требуется для E2EE и бэкапа ключей
      key_backup: true                   # Резервное копирование ключей шифрования на сервере Matrix
  ...
  - id: "system_tasks"
    items:
      ...
      - name: "weather_workdays"
        ...
        script: "weather.sh"      #  Локальный скрипт имеет приоритет (если заполнен, url игнорируется)
        # url: "https://wttr.in/Moscow?format=3"  # Вызывается, только если параметр script пуст
...
- `retry_count` / `retry_delay` – начальные параметры повторов (если ≤0, подставляются 3 и 2). Максимальная пауза экспоненциального бэкоффа ретраев очереди ограничена переменной окружения `MAX_RETRY_DELAY` в `docker-compose.yml` (по умолчанию `300` секунд / 5 минут), чтобы сообщения не зависали на долгие часы при обрывах связи. При первой же успешной доставке любого сообщения канала задержки накопившейся очереди мгновенно сбрасываются.
...
- `show_time` - (default: false) добавлять метку времени получения сообщения ботом (YYYY-MM-DD HH:MM:SS MST) в конец текста при первичной успешной доставке. Не влияет на префикс `[Отложенная доставка]`, который подставляется ВСЕГДА при повторных попытках.
- `encryption` - активация сквозного шифрования для Matrix.
- `recovery_key` - ключ восстановления Matrix (Recovery Key), необходимый для расшифровки/шифрования бэкапа ключей на сервере и инициализации E2EE-сессии.
- `key_backup` - (опционально) включение резервного копирования и восстановления ключей шифрования (Megolm-сессий) на сервере Matrix. Позволяет автоматически восстановить доступ к истории зашифрованных чатов при потере локальной SQLite базы данных бота.
...
- `tasks` - уникальный идентификатор подключенного списка периодических задач по расписанию (cron).
- `schedule` - выражение расписания запуска задачи в стандартном формате cron (например, `*/5 * * * *`).
- `enabled` - (опционально для задач) флаг активности конкретной периодической задачи (`true`/`false`). По умолчанию `true`. Если установлен в `false`, задача полностью игнорируется при планировании.
- `url` - (опционально для задач и меню) URL-адрес внешнего ресурса для запроса. Для задач cron вызывается, только если параметр `script` пуст.
- `description` - (опционально для задач и меню) текстовое описание задачи или команды меню для логирования и документирования.
...
## Планировщик задач по расписанию (Cron Scheduler)

Начиная с версии `3.4.0`, бот поддерживает возможность периодического автоматического запуска локальных скриптов или выполнения внешних HTTP-запросов по расписанию (cron).

Результаты выполнения отправляются в чаты Telegram/Matrix подключенных инстансов с поддержкой правил гарантированной доставки (Outbox + TTL) и автоматического распознавания/передачи сгенерированных файлов или отчетов.

### 1. Как это работает

1. **Глобальные списки задач (`tasks`)**: В корневом конфигурационном файле `config.yml` описываются один или несколько именованных списков задач.
2. **Привязка к инстансу**: Инстансы подключают нужный список задач через параметр `tasks: <id_списка>`.
3. **Бесшовный перезапуск (Hot-Reload)**: При изменении расписания, добавлении/удалении задач или изменении флага активности (`enabled`), бот динамически останавливает старые задачи инстанса и запускает новые без прерывания доставки других уведомлений.

### 2. Параметры задачи в конфигурации

Каждая задача может содержать следующие параметры:
* `name` (string, обязательно) — уникальное имя задачи для логирования и идентификации.
* `schedule` (string, обязательно) — расписание запуска в стандартном формате cron (например, `*/5 * * * *` для запуска каждые 5 минут).
* `script` (string, опционально) — имя локального исполняемого скрипта из каталога `/app/scripts/`. Если задан, имеет приоритет перед `url`.
* `url` (string, опционально) — URL-адрес для выполнения HTTP GET запроса (например, для вызова внешнего API, возвращающего текстовый или графический отчет. Например, запрос `https://wttr.in/Moscow?format=3` вернет текстовый прогноз погоды для Москвы).
* `description` (string, опционально) — текстовое описание задачи для логирования.
* `enabled` (boolean, опционально) — флаг активности задачи. По умолчанию `true`. Если выставлен в `false`, задача полностью игнорируется при планировании и не будет запускаться.

### 3. Обработка ответов и отправка файлов

Планировщик обрабатывает вывод запущенных скриптов или ответы от HTTP-запросов аналогично интерактивному меню:
* **Текст / JSON / XML**: Обычные текстовые выводы отправляются в чат как сообщения. JSON и XML ответы красиво форматируются.
* **Передача бинарных файлов (изображения/документы)**: Если stdout скрипта является бинарным потоком (например, генерируется картинка или PDF-документ), бот автоматически сохраняет его и отправляет в чаты как вложение.
* **Передача файлов через JSON-вывод**: Если скрипт возвращает JSON-структуру с полем `file_path` или `file` (например, `{"file_path": "/app/data/report.pdf", "text": "Ежедневный отчет"}`), бот копирует этот файл в медиа-директорию для монопольного владения, определяет MIME-тип и отправляет как вложение с подписью из поля `text`.

### 4. Таймауты выполнения
* Для локальных скриптов время выполнения ограничено переменной окружения `SCRIPT_TIMEOUT` (по умолчанию `15` секунд).
* Для внешних HTTP GET запросов время выполнения ограничено переменной окружения `HTTP_REQUEST_TIMEOUT` (по умолчанию `15` секунд).

### 5. Гарантированная доставка (Outbox)

Все сформированные планировщиком сообщения регистрируются в SQLite со статусом `pending` для каждого активного сервиса инстанса (Telegram/Matrix). Это гарантирует, что даже если в момент срабатывания cron-задачи отсутствовал интернет или были перебои с серверами Telegram/Matrix:
* Сообщения сохранятся в локальной очереди.
* Будут предприняты повторные попытки доставки с использованием экспоненциального backoff.
* Временные медиафайлы не удалятся с диска, пока все воркеры не подтвердят успешную отправку во все каналы.
...
### Резервное копирование ключей E2EE на сервере (Matrix Key Backup)

Начиная с версии `3.5.0`, бот поддерживает механизм **Server-Side Key Backup** (в соответствии со спецификацией Matrix MSC1219). Это позволяет автоматически восстанавливать исторические ключи шифрования (Megolm-сессии) из Homeserver при утере локальной базы данных SQLite бота (например, при пересоздании контейнеров без монтирования volumes).

#### Принцип работы:
1. **Восстановление (Restore):** При старте бота, если файл локальной БД `<account_id>.db` пуст, бот автоматически обращается к Homeserver, находит активную версию бэкапа, расшифровывает её с помощью приватного ключа `recovery_key` и импортирует ключи в SQLite. Бот мгновенно готов расшифровывать новые сообщения в зашифрованных комнатах.
2. **Фоновый импорт существующих ключей:** При старте бота с уже накопленными локальными ключами запускается асинхронная задача, которая шифрует все локальные ключи и выгружает их пакетами по 100 сессий на Homeserver.
3. **Автоматический бэкап новых ключей:** Любой новый ключ шифрования, полученный в процессе работы (например, при ротации сессий или получении новых ключей комнат), перехватывается специальным хранилищем-враппером и сразу же асинхронно выгружается на сервер в фоновом режиме.

#### Настройка резервного копирования:
Для включения механизма в блоке `matrix` должен быть указан корректный `recovery_key` (Base58 фраза, генерируемая при создании резервной копии в Element), а также активирован сам бэкап:

* **Через файл конфигурации (`config.yml`):**
  ```yaml
  matrix:
    enabled: true
    encryption: true
    recovery_key: "Es3t 9A2t 1B3c ..." # Ваша фраза восстановления
    key_backup: true
  ```
* **Через переменные окружения (имеет наивысший приоритет):**
  Установите переменную среды `MATRIX_ENABLE_KEY_BACKUP=true` (или `false` для принудительного отключения).

> [!IMPORTANT]
> После изменения параметров Key Backup (в `config.yml` или через переменные окружения) требуется **обязательный перезапуск контейнера бота** (`docker compose restart notify-bot`). Горячая перезагрузка конфигурации (Hot-Reload) не переинициализирует запущенные криптографические клиенты E2EE на лету.
