...
- **Гарантированная доставка и ограничение задержек (`MAX_RETRY_DELAY`)**: Все сообщения сохраняются в SQLite перед отправкой. Задержка повторных попыток регулируется переменной `MAX_RETRY_DELAY` (по умолчанию 300 секунд / 5 минут), исключая длительное зависание очереди при сетевых сбоях.
- **Мгновенный сброс задержки профиля чата**: При успешной доставке любого сообщения из очереди конкретного инстанса и сервиса, воркер автоматически сбрасывает `next_attempt_at` для остальных неотправленных сообщений этого чата и высылает весь накопившийся пакет сообщений.
...
- **Сквозное шифрование (Matrix E2EE)**: Поддержка зашифрованных комнат Matrix с хранением состояния в SQLite и Server-Side Key Backup.
