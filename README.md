...
tasks:
  - id: "system_tasks"
    items:
      ...
      - name: "weather_report"
        schedule: "0 8 * * *"    # Выполнять каждый день в 8:00 утра
        script: ""               # Локальный скрипт имеет приоритет (если заполнен, url игнорируется)
