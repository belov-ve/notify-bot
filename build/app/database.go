...
type Message struct {
	...
	CreatedAt    time.Time
	FilePath     string // [NEW] Путь к локально сохраненному файлу на диске
	FileName     string // [NEW] Оригинальное имя файла
	MimeType     string // [NEW] MIME-тип файла
}
...
func (db *DBWrapper) SaveMessage(m *Message) error {
	// [NEW] Записываем новые поля вложений (file_path, file_name, mime_type) в БД.
...
func (db *DBWrapper) GetPendingMessages() ([]Message, error) {
	// [NEW] Выбираем поля file_path, file_name, mime_type для отправки вложений.
